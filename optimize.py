...
def main():
    try:
        ...
        print("-" * 80)
        print(f"{'LR':<6} {'Iter':<6} {'R²':<10}"
              f"{'MSE':<12} {'RMSE':<10} {'MAE':<10}")
        print("-" * 80)
        ...
        for lr in learning_rates:
            for iterations in iterations_list:
                ...
                print(f"{lr:<6.2f} {iterations:<6d} {metrics['r2']:<10.6f} "
                      f"{metrics['mse']:<12.2f} {metrics['rmse']:<10.2f} "
                      f"{metrics['mae']:<10.2f}")
        ...
        for r in results:
            ...
            r2_norm = ((r['r2'] - r2_min) / (r2_max - r2_min)
                       if r2_max != r2_min else 1.0)

            # Normalize and invert MSE, RMSE, MAE (lower is better)
            mse_norm = (1.0 - (r['mse'] - mse_min) / (mse_max - mse_min)
                        if mse_max != mse_min else 1.0)
            rmse_norm = (1.0 - (r['rmse'] - rmse_min) / (rmse_max - rmse_min)
                         if rmse_max != rmse_min else 1.0)
            mae_norm = (1.0 - (r['mae'] - mae_min) / (mae_max - mae_min)
                        if mae_max != mae_min else 1.0)

            # Composite score (R² weighted more heavily)
            r['composite_score'] = (0.5 * r2_norm + 0.2 * mse_norm
                                    + 0.15 * rmse_norm + 0.15 * mae_norm)
        ...
        print("-" * 80)
        print(f"\n✓ Best parameters (composite score: {best_score:.4f}):")
        ...
        ax1.plot(best_j, best_i, 'b*', markersize=20,
